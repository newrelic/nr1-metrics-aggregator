...
import { AutoSizer, Grid, GridItem, HeadingText, NerdGraphQuery, Toast } from "nr1";
...
export default class AddE2MRule extends React.Component {
	constructor(props) {
		...
		this.buildOrGetCreateRuleNRQL = this.buildOrGetCreateRuleNRQL.bind(
			this
		);
		this.ruleIsCompletelyValidated = this.ruleIsCompletelyValidated.bind(
			this
		);
	...
	buildOrGetCreateRuleNRQL() {
		...
		}(\`${this.state.selectedAttribute}\`) AS '${
	...
	async createValidatedRule() {
		...
		let { result, error } = await createAndSaveNewRule(
			selectedAccountID,
			nrql,
			validatedRuleAlias,
			validatedRuleDescription
	...
	readyForStepThree() {
		...
		return (
			this.readyForStepTwo() && validatedRuleAlias && validatedMetricName
		);
	}
					...
					<div className="outerFormWrapper">
						<div className="innerFormWrapper">
							...
							<Section3
								...
								selectedAccountID={selectedAccountID}
								ruleIsCompletelyValidated={
									this.ruleIsCompletelyValidated
								}
