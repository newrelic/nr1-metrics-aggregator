...
class RuleValidator extends React.Component {
	...
	render() {
		...
		if (
			...
			this.props.validatingCardinality
		) {
			return <Spinner />;
		}
		if (
			...
			return (
				<div>
					Please wait... Still downloading cardinality details for all
					current rules on this account.
					<Spinner />
				</div>
			);
		}
				...
				<ul className="violationList">
					{this.props.cardinalityRuleViolation ? (
						<li className="CardinalityViolation">
							<b>
								Your rule has more than 20,000 unique values
								(per-rule cardinality violation)!
							</b>
							<br />
							<span className="detail">
								You must remove attributes and/or narrow the
								scope to proceed.
							</span>
						</li>
					) : (
						<li className="NoCardinalityViolation">
							Your rule is within the per-rule cardinality limit
							of 20,000 over the last week.
						</li>
					)}
					{this.props.cardinalityAccountViolation ? (
						<li className="CardinalityViolation">
							<b>
								Your account has over 5 million unique custom
								values (account cardinality violation)!
							</b>
							<br />
							<span className="detail">
								You must remove other rules or modify the
								attributes selected in this rule to proceed.
							</span>
						</li>
					) : (
						<li className="NoCardinalityViolation">
							This rule does not push your account above the 5M
							total account cardinality limit.
						</li>
					)}
				</ul>
