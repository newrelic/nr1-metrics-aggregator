...
class MetricNameValidator extends React.Component {
	...
	setMetricName(userInputtedMetricName) {
		...
		const metricName = userInputtedMetricName.replace(
			/[^a-zA-Z0-9\.]/g,
			""
		);
	...
	getSuggestedMetricName() {
		...
		let name = `${this.props.selectedEventType}.${
			this.props.selectedAttribute
		}.${this.props.selectedAggregator}`;
		...
		return (
			<div className="validator">
				...
				<div
					...
					onClick={() =>
						this.setMetricName(this.getSuggestedMetricName())
					}
				>
					Use suggested name
				</div>
