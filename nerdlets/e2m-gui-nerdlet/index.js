...
export default class E2mGui extends React.Component {
	constructor(props) {
		...
		this.addNewRuleAndCloseDialog = this.addNewRuleAndCloseDialog.bind(
			this
		);
	...
	setCardinalitiesForAccount(cardinalitiesForAccount, accountId, e2mRules) {
		...
		const cardinalities = copied.filter(
		...
		let ruleCount = cardinalities.reduce(
	...
	async loadCardinality(e2mRules, accountsObj, noCache = false) {
		...
		try {
			...
			let cardinalities = [];
			let cardinalityTotals = {};
			for (let i = 0; i < accountIds.length; i++) {
				const accountId = accountIds[i];
				let cardinalitiesForAccount = await loadCardinalityForAllEnabledRules(
					e2mRules,
					accountId,
					cardinalities =>
						this.setCardinalitiesForAccount(
							cardinalities,
							accountId,
							e2mRules
						),
			...
			if (!timerangeArray || !timerangeArray.length) {
				const nonEmptyCardinalities = cardinalities.filter(
					card =>
						card["cardinalities"] && card["cardinalities"].length
				);
								...
								>
									{!accountsObj ? (
										...
										<AddE2MRule
											...
											addNewRuleAndCloseDialog={
												this.addNewRuleAndCloseDialog
											}
										/>
									)}
								</div>
