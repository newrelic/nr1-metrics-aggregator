...
export default class E2mGui extends React.Component {
  ...
  setCardinalities(cardinalities, cardinalityTotals, timerangeArray) {
    const { timerangeArray: oldTimerangeArray } = this.state;
    const timerange =
      !oldTimerangeArray || !oldTimerangeArray.length
        ? timerangeArray
        : oldTimerangeArray;
    this.setState({
      cardinalities,
  ...
  setCardinalitiesForAccount(cardinalitiesForAccount, accountId, e2mRules) {
    const {
      cardinalities: oldCardinalities,
      timerangeArray: oldTimerangeArray
    } = this.state;
    const copied =
      !oldCardinalities || !oldCardinalities.length
        ? []
        : [...oldCardinalities];
  ...
  async loadCardinality(e2mRules, accountsObj, noCache = false) {
    ...
    try {
      ...
      this.setState({ cardinalityDataLoading: false });
    }
  }
