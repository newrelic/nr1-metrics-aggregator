...
export async function calculateVolumeReductionForMetric(metric) {
	...
	if (!errors) {
		try {
			eventRate =
				data["actor"]["EventRate"]["nrql"]["results"][0]["eventrate"];
			metricRate =
				data["actor"]["MetricRate"]["nrql"]["results"][0]["metricrate"];
		} catch (err) {
			errors = err;
		}
	}
...
export async function createAndSaveNewRule(
	...
	return await storeRuleCardinalityIfOtherCardinalityStored(
		accountId,
		newRule
	);
}
...
async function calculateCardinalityForEnabledRulesForAccount(
	...
	for (let i = 0; i < ruleBatchArray.length; i++) {
		try {
			...
			let { data, errors } = await calculateCardinalityForRuleBatch(
				batchOfRules,
				accountId
			);
			if (errors) {
...
export async function findRuleViolations(
	...
	let { data, error } = await NerdGraphQuery.query(
		buildCardinalityTimeseriesQuery(
			selectedAccountID,
			selectedEventType,
	...
	const results =
		data["actor"][`query${selectedAccountID}`]["nrql"]["results"];
...
export async function toggleMetric(ruleId, accountId, enabled) {
	const mutation = buildToggleRuleQuery(
		accountId,
		ruleId,
		!enabled
	);
